...
$(document).ready(() => {
  const links = [
    {
      name: 'all my homies use ice hub ♥',
      link: '76561197960276740',
    },
    {
      name: 'pls dont skid xd',
      link: '76561198193201687',
    },
    {
      name: 'can you stop looking here',
  ...
  for (let i in links) {
    ...
    $('#marquee').append(`<a href="unity.com/profiles/${link.link}" target="_BLANK">${link.name}</a>`);
  ...
  app.titleChanger(['<3 Thanks for using icehub', 'by oksavor', 'lol', 'all my homies use icehub frfr', 'synapse x only brokie', '=', '21 savage fanboys', 'lez get it boiiis']);
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[unknown]</span>..."], 30, () => {
    ...
    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
      ...
      writeLine([`<i style='color: #F62459'>icehub $$$</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
